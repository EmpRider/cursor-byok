...
const resolvedTitle = computed(() => {
  ...
  if (!text || text === "测试" || text.toLowerCase() === "test") {
    return String(localized("6aa8f49cc992dfd7", "Test"));
  }
  return text;
});

const resolvedEmptyText = computed(() => {
  const text = String(props.emptyText || "").trim();
  if (!text || text === "未测试" || text.toLowerCase() === "not tested") {
    return String(localized("092b520558eff5f2", "Not tested"));
  }
  return text;
});
